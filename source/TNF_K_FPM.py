...
class NBFK_FPM(HybridODE):
    ...
    def forward(self, t, x, *args, **kwargs):
        ...
        dx4 = -x4 + 0.5 * self.act(u.squeeze(-1), self.b1) 
...
if __name__ == "__main__":  
    ...
    observed_state_idx = [0,1,2,3] # Only 3rd state observed
    ...
    # Fermentor FPM:
    nbf_ode = NBFK_FPM(mech_params = mech_param_dict, mode = "physical")
    ...
    if not inference_mode:
        max_epochs = 200
        trainer2 = pl.Trainer(max_epochs=max_epochs, enable_checkpointing=False, logger = False, accelerator='cpu')
        # Train this first
        trainer2.fit(learner_hybrid, dataloader)   
    
    else:    
        nbf_ode2 = NBFK_FPM(mech_params = mech_param_dict, mode = "inference")
